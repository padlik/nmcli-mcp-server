# vpn-management Specification

## Purpose

Semantic, allowlisted VPN control over NetworkManager for LLM agents: seven fixed
MCP tools (list, status, diagnose, network overview, idempotent connect,
disconnect, health-aware reconnect) instead of arbitrary shell access. VPN ids
resolve to NetworkManager connection names from validated TOML config; all
subprocess execution is fixed-argv without a shell, with structured JSON results
(including timeouts) and local-only usability diagnosis.
